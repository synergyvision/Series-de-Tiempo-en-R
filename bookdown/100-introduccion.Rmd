...
```{definition, defi-tendencia}
La **tendencia**, se define como una función $T_t$ de $t$ que describe la evolución lenta y a largo plazo del nivel medio de la serie. La función $T_t$ depende de parámetros, que deben estimarse.
```
...
```{definition, defi-modelo-log-lineal_1}
...
```{example, ejem-metodo-T1}
Ajustando una función de la forma \@ref(eq:eq-modelo-cuadratico) para la población de los datos en el gráfico \ref{Grafico-Tema3-poblacion-USA} para $1790\leq t\leq1980$ nos da los parámetros estimados
...
para la población de EE.UU en 1990. Sin embargo si los residuales $\hat{\epsilon}_t$ están altamente correlacionados podemos ser capaces de usar esos valores para dar una mejor estimación de $\epsilon_{1990}$ y por consiguiente de $X_{1990}$.
```
