...
par(mfrow=c(2,1))
hist(gnpgr.arima$residuals)
qqnorm(gnpgr.arima$residuals)
shapiro.test(gnpgr.arima$residuals)
```
...
```{r,fig.cap="Población de los EE.UU (puntos azules) y modelos ajustado (linea roja continua) de 1910 hasta 2010",fig-sobreajuste-poblacion-usa}
...
Tabla=matrix(numeric(9),3,3)
# AIC
Tabla[1,1]=log(sar)+(n+2*kar)/n
Tabla[2,1]=log(sma)+(n+2*kma)/n
Tabla[3,1]=log(sarima)+(n+2*karima)/n
# AICc
Tabla[3,2]=log(sar)+(n+kar)/(n-kar-2)
Tabla[3,2]=log(sma)+(n+kma)/(n-kma-2)
Tabla[3,2]=log(sarima)+(n+karima)/(n-karima-2)
# BIC (SIC)
Tabla[3,2]=log(sar)+kar*log(n)/n
Tabla[3,2]=log(sma)+kma*log(n)/n
Tabla[3,2]=log(sarima)+karima*log(n)/n
# Nombre de las columnas
Tabla<-as.data.frame(Tabla)
colnames(Tabla)=c("AIC","AICc","SIC")
rownames(Tabla)=c("AR(1)","MA(2)","ARIMA(1,1,2)")
Tabla
```
----
