...
```{example, name="Temperatura global", ejem-temperatura-global}
...
```{example, name="Uso de regresión para descubrir una señal de ruido", ejem-regresion-senal-ruido}
...
```{r, fig.cap="Datos generados por el modelo senosoidal [línea punteado roja] con modelo de regresión [línea solida azul", fig-tema3-regresion-senal-ruido} 
...
```{example, name="Uso de periodograma para descubrir una señal de ruido", ejem-periodograma-senal-ruido}
...
```{r, name="Periodograma de 500 observaciones generadas por el modelo senosoidal", fig-tema3-periodograma-modelo-senosoidal}
t=1:500
...
Los estimadores que se usan con más frecuencia para estimar los modelos $ARCH$ son los que se obtienen de la función de máxima verosimilitud gaussiana condicional. Consideremos el modelo $ARCH(p)$ dado en la ecuación \@ref(eq:eq-modelo-ARCHp). Bao la hipótesis de normalidad de $\nu_t$, tenemos que la distribución de $w_t$ condicionado a $\mathcal{F}_{t-1}$, tiene una distribución normal co media $\mu=0$ y varianza $h_t=0$, y la función de máxima verosimilitud adopta la forma

\begin{eqnarray}
f(w_1,\ldots,w_T|\vec{\alpha}) &=& f(w_T|\mathcal{F}_{T-1})f(w_T-1|\mathcal{F}_{T-2})\cdots f(w_{p+1}|\mathc{F}_p)f(w_1,\ldots,w_p|\vec{\alpha}) \nonumber\\
...
En efecto, bajjo la suposición de estacionaridad se tiene
...
```{example, name="Una serie periódica", ejem-serie-periodica}
...
```{r, fig.cap="Componentes periódicas y su suma como se describe en el Ejemplo", fig-componentes-periodicas}
...
```{example, name="Periodograma escalado para el ejemplo anterior", ejem-periodograma-escalado}
...
```{r, fig.cap="Periodograma de los datos generados en el Ejemplo ...", fig-periodograma-escalado}
...
```{example, name="Un proceso estacionario periódico", ejem-proceso-estacionario-periodico}
...
```{proposition, name="La Densidad Espectral", propie-densidad-espectral}
...
```{example, name="Serie de ruido blanco", ejem-espectro-serie-ruido-blanco}
...
```{example, name="Un promedio móvil simple", ejem-espectro-promedio-movil-simple}
...
```{example, name="Una serie autoregresiva de segundo orden", ejem-espectro-AR2}
...
```{r, fig.cap="Espectros teóricos de un ruido blanco (superior), promedio móvil de ruido blanco (medio) y proceso AR(2) (inferior)", fig-espectros-teoricos}
n=100
sigma2=1
...
```{example, name="ANOVA espectral", ejem-ANOVA-espectral}
...
```{example, name="Covarianzas de senos y cosenos para un proceso MA", ejem-cov-seno-coseno-MA}
...
```{proposition, name="Distribución de las Ordenadas de un Periodograma", propie-distrib-ordenadas-periodograma}
Si
...
```{example, name="Periodograma de SOI y serie de reclutamiento (nuevos peces)", ejem-periodograma-SOI}
...
```{r, fig.cap="Periodograma de SOI y Reclutamiento (nuevos peces)", fig-periodograma-SOI}
...
```{example, name="Periodograma suavizado de las series SOI y reclutamiento (nuevos peces)", ejem-periodograma-suavizado-SOI}
...
```{r echo=FALSE, fig.cap="Periodograma suavizado de las series SOI y Reclutamiento", fig.align="center", fig-periodograma-suavizado-SOI}
knitr::include_graphics("images/ejemplo4p10.png")
```

---

```{example, name="Serie de Alturas de Olas. Estación 144. ST. PETERSBURG", ejem-espectro-altura-olas}
...
```{r, fig.cap="Periodograma para las alturas de olas, Estación 144, St. Petersburg, FL.", fig-periodograma-altura-olas}
...
```{example, name="Periodogramas para las series de Terremotos y Explosiones", ejem-periodograma-terremoto-explosiones}
...
```{r, fig.cap="Periodogramas para las series de Terremotos y Explosiones", fig-periodograma-terremoto-explosiones}
...
```{theorem, name="Teorema de Herglotz", teo-Herglotz}
...
```{remark, obser-prop-incremento-ortogonal}
...
```{theorem, name="El Teorema de Representación Espectral", teo-representacion-espectral}
...
```{example, name="Representación espectral de un movimiento browniano", ejem-representacion-espectral-movimiento-browniano}
