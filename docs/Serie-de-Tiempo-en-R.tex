...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-8-1} 
...
\{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-9-1}
...
\{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-10-1}
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-11-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-12-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-13-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-14-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-15-1} 
...
{\centering \includegraphics[width=8.12in]{images/Grafica-tema3-modelo-aditivo} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-17-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-18-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-19-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-38-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-39-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-40-1} 
...
\includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-41-1.pdf}
\includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-41-2.pdf}
...
\includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-42-1.pdf}
...
El AIC no proporciona una prueba de un modelo en el sentido de probar
una hipótesis nula, es decir el AIC puede decir nada acerca de la
calidad del modelo en un sentido absoluto. Si todos los modelos
candidatos encaja mal, el AIC nodará ningún aviso de ello. El valor de
\(k\) que minimiza el AIC especifica el mejor modelo. La idea es que la
minimización de \(\hat{\sigma}_k^2\) sea razonablemente objetiva,
excepto que decrezca monótonamente cuando \(k\) crece. Por lo tanto,
debemos penalizar la variación del error por un término proporcional al
número de parámetros. La elección del término de penalización dado por
\eqref{eq:eq-AIC} no es único.

En el criterio AIC definido en la ecuación \eqref{eq:eq-AIC}, el sesgo es
aproximado por el número de parámetros los cuales son constantes y no
tienen variabilidad. Para el modelo de regresión, la corrección del
sesgo el logaritmo de la verosimilitud se define como

\BeginKnitrBlock{definition}
\protect\hypertarget{def:defi-AICc}{}{\label{def:defi-AICc} }
...
donde \(\hat{\sigma}_k^2\) es el estimador de máxima verosimilitud de la
varianza, \(k\) es el número de parámetros en el modelo y \(n\) es el
tamaño de la muestra.
\EndKnitrBlock{definition}

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

Este modelo fue propuesto originalmente por \emph{N. Sugiura} en el
artículo \textbf{``Further analysis of the data by Akaike's information
criterion and the finite corrections''}. \emph{Communications in
Statistics, Theory and Methods}, Vol. 7, No. 1, pp.~13-26, 1978. Se
tiene que el AICc es esencialmente el AIC con un término de penalización
adicional para el número de parámetros. Nótese que cuando
\(n\to\infty\), el término de penalización adicional converge a 0, y por
lo tanto el AICc converge al AIC. De manera similar que en el AIC, se
selecciona el modelo con el menor valor AICc.

\BeginKnitrBlock{definition}
\protect\hypertarget{def:defi-SIC}{}{\label{def:defi-SIC} }
...
SIC también llamado \emph{Criterio Bayesiano de Información} (BIC) es un
criterio para la selección de modelos. Se basa, en parte, en la función
de probabilidad y está estrechamente relacionado con el Criterio de
Información de Akaike (AIC). Cuando se hace ajuste de modelos, es
posible aumentar la probabilidad mediante la adición de parámetros, pero
esto puede resultar en sobreajuste. Tanto BIC como AIc resuelven este
problema mediante la introducción de un término de penalización para el
núemro de parámetros en el modelo, el término de penalización es mayor
en el BIC que en el AIC.

El BIC fue desarrollado por \emph{Gideon E. Schwarz}, quien dio un
argumento bayesiano a favor de su adopción. Akaike también desarrolló su
propio formalismo bayesiano, que ahora se conoce como la ABIC (siglas en
inglés) por Criterio de Información Bayesiano de Akaike.

\BeginKnitrBlock{example}
\protect\hypertarget{exm:ejem-serie-nuevos-peces-AR2}{}{\label{exm:ejem-serie-nuevos-peces-AR2}
}Consideremos el archivo de datos ``recruit.txt'', consistente del
número de nuevos peces en el Pacífico Central relacionado con el Índice
de Oscilación del Sur (SOI) que mide los cambios en la presión del aire
relativo a la temperatura de la superficie del mar. Son 453 registros de
1950 a 1987 tomados mensualmente. La ACF y la PACF indican que el modelo
que mejor se ajusta es un \(AR(2)\). La ACF tiene ciclos
correspondientes a aproximadamente períodos de 12 meses y la PACF tiene
valores grandes para \(h=1,2\) y es esencialmente cero para paso de
orden mayor.
\EndKnitrBlock{example}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rec=}\KeywordTok{ts}\NormalTok{(}\KeywordTok{scan}\NormalTok{(}\StringTok{"data/recruit.txt"}\NormalTok{),}\DataTypeTok{start =} \DecValTok{1950}\NormalTok{, }\DataTypeTok{frequency =} \DecValTok{12}\NormalTok{)}
\KeywordTok{par}\NormalTok{(}\DataTypeTok{mfrow=}\KeywordTok{c}\NormalTok{(}\DecValTok{3}\NormalTok{,}\DecValTok{1}\NormalTok{))}
\KeywordTok{plot}\NormalTok{(rec,}\DataTypeTok{type =} \StringTok{"l"}\NormalTok{, }\DataTypeTok{ylab =} \StringTok{""}\NormalTok{, }\DataTypeTok{xlab =} \StringTok{"meses"}\NormalTok{, }\DataTypeTok{main =} \StringTok{"Número de nuevos peces en el Pacífico Central (1950-1987)"}\NormalTok{)}
\KeywordTok{acf}\NormalTok{(rec,}\DecValTok{48}\NormalTok{)}
\KeywordTok{pacf}\NormalTok{(rec,}\DecValTok{48}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-43-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Ajuste AR(2)}
\NormalTok{regr=}\KeywordTok{ar.ols}\NormalTok{(rec,}\DataTypeTok{order=}\DecValTok{2}\NormalTok{,}\DataTypeTok{demean =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{intercept =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{regr}\OperatorTok{$}\NormalTok{asy.se.coef}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ts.plot}\NormalTok{(rec,fore}\OperatorTok{$}\NormalTok{pred,}\DataTypeTok{col=}\DecValTok{1}\OperatorTok{:}\DecValTok{2}\NormalTok{,}\DataTypeTok{xlim=}\KeywordTok{c}\NormalTok{(}\DecValTok{1980}\NormalTok{,}\DecValTok{1990}\NormalTok{),}
        \DataTypeTok{xlab=}\StringTok{"Años"}\NormalTok{, }\DataTypeTok{ylab=}\StringTok{"Nuevos peces"}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fore}\OperatorTok{$}\NormalTok{pred,}\DataTypeTok{type =} \StringTok{"p"}\NormalTok{,}\DataTypeTok{col=}\DecValTok{2}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fore}\OperatorTok{$}\NormalTok{pred}\OperatorTok{+}\NormalTok{fore}\OperatorTok{$}\NormalTok{se,}\DataTypeTok{lty =} \StringTok{"dashed"}\NormalTok{,}\DataTypeTok{col=}\DecValTok{4}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fore}\OperatorTok{$}\NormalTok{pred}\OperatorTok{-}\NormalTok{fore}\OperatorTok{$}\NormalTok{se,}\DataTypeTok{lty =} \StringTok{"dashed"}\NormalTok{,}\DataTypeTok{col=}\DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-44-1.pdf}

\chapter{Modelos MA}\label{modelos-ma}

En este capítulo describiremos otra clase de modelos simples que también
son útiles en le modelado de series de retorno en finanzas. Estos
modelos se denominam mdodelos de promedio móvil (\(MA\), siglas en
ingles: Moving Average). Hay varias maneras de introducir los modelos
\(MA\). Un enfoque es tratar el modelo como una extensión de una serie
de ruido blanco; alternativamente a la representación autoregresiva en
la cual \(x_t\) del lado izquierdo se asume como una combinación lineal,
en los modelos de promedio móvil e orden \(q\) \(MA(q)\), asumimos el
ruido blanco \(w_t\) del lado derecho de la ecuación que los define como
una combinación lineal de los datos observados.

\BeginKnitrBlock{definition}
\protect\hypertarget{def:defi-modelo-MAq}{}{\label{def:defi-modelo-MAq} }El
\textbf{modelo de promedio móvil de orden \(q\)} o modelo \(MA(q)\), se
define como
...
donde hay \(q\) pasos o saltos en el promedio móvil y
\(\theta_1,\theta_2,\ldots,\theta_q (\theta_q\neq0)\) son parámetros.
\footnote{Algunos libros y algunos paquetes estadísticos escriben el
  modelo \(MA\) con coeficientes negativos, esto es
  \[x_t=w_t-\theta_1x_{t-1}-\theta_2x_{t-2}-\cdots-\theta_qx_{t-q}\]} El
ruido \(w_t\) se asume como un ruido blanco gaussiano.
\EndKnitrBlock{definition}
...
\begin{equation}
x_t = \theta(B)w_t
\label{eq:eq-MAq-operador}
\end{equation}

usando la siguiente definición.

\BeginKnitrBlock{definition}
\protect\hypertarget{def:defi-operador-proemdio-movil}{}{\label{def:defi-operador-proemdio-movil}
}El \textbf{operador de promedio móvil} se define como

\begin{equation}
\theta(B) = 1+\theta_1B+\theta_2B^2+\cdots+\theta_qb^q
\label{eq:eq-operador-promedio-movil}
\end{equation}
\EndKnitrBlock{definition}

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

A diferencia del proceso autoregresivo \(AR(p)\), el proceso de promedio
móvil \(MA(q)\) es estacionario para cada valor de los parámetros
\(\theta_1,\theta_2,\ldots,\theta_q\).

Otro enfoque para introducir los proceso de promedio móvil es tratar el
modelo como un modelo \(AR\) de orden infinito con algunas
restricciones. No hay una razón en particular, pero por simplicidad y
sin pérdida de generalidad, asumiremos \emph{a priori} que el orden del
modelo es finito. Podemos suponer, al menos en teoría, un modelo \(AR\)
con orden infinito como

\[x_t=\phi_0+\phi_1x_{t-1}+\phi_2x_{t-2}+\cdots+w_t\] Sin embargo, tal
modelo \(AR\) no esrealista porque tiene infinitos parámetros. Una forma
de hacer práctico el modelo es asumir que los coeficientes \(\phi_i\)
satisfacen algunas restricciones para que sean determinados por un
número finito de parámetros. Un caso especial de esta idea es
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-45-1} 
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-46-1} 
...
En este caso, la ACF corta en paso 2. Esta propiedad la podemos
generalizar a los modelos \(MA(q)\). Así, para un modelo \(MA(q)\) la
ACF se corta en paso \(q\), y vale cero para \(h>q\). Consecuentemente,
una serie \(MA(q)\) está solo linealmente relacionada con sus \(q\)
primeros valores y por consiguiente es un modelo de ``memoria finita''.

\BeginKnitrBlock{example}
\protect\hypertarget{exm:ejem-no-unicidad-MA}{}{\label{exm:ejem-no-unicidad-MA}
}USando las funciones de autocovarianza (ec.
\eqref{eq:eq-autocovarianza-MA1}) y de autocorrelación (ec.
\eqref{eq:eq-ACF-MA1}) de un modelo \(MA(1)\) podemos notar que
\(\rho(h)\) es el mismo para \(\theta\) y \(1/\theta\), probemos, por
ejemplo, con \(\theta=5\) y \(\frac{1}{\theta}=\frac{1}{5}\)
...
Además, el par \(\sigma_w^2=1\) y \(\theta=5\) llevan a la misma función
de autocovarianza que el par \(\sigma_w^2=25\) y \(\theta=1/5\), esto es
...
{\centering \includegraphics{Serie-de-Tiempo-en-R_files/figure-latex/unnamed-chunk-47-1} 
